...
from src.services.planning.posting_windows import DAY_PARTS, build_posting_plan
...
class GrowthEngine(BaseCollector):
    ...
    # sourced from the shared planning module so the campaign fallback stays in sync.
    ...
    def _build_posting_plan(self, posts_per_day, hourly_all) -> list[dict] | None:
        ...
        Thin wrapper over the shared builder so the campaign planner can reuse the
        exact same logic for its history-derived fallback."""
        return build_posting_plan(posts_per_day, hourly_all)
