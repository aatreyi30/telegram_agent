...
DAILY_INSTRUCTIONS = (
    ...
    "4. If a section has no supporting data, omit it and say so briefly rather than "
    "padding it out.\n\n"

    "OUTPUT FORMAT: Freeform prose (not JSON, no headers), in this exact order: headline, "
    "what changed & why, do today (the top 2-3 recommendations with their numbers). "
...
WEEKLY_INSTRUCTIONS = (
    ...
    "padding it out.\n"
    "6. When per-day follower deltas (joined/left/net) are present in the DATA, factor "
    ...
    "7. When 'retro' is present in the DATA, its `metrics.adjustments` are concrete, "
    "rule-based changes for next week (e.g. shift posting hours toward a bucket, reduce "
    "a merchant/type) derived from last week's prediction accuracy and engagement data. "
    "For EACH adjustment, explicitly say whether you are incorporating it into 'what to "
    "change next week' or rejecting it and why — never silently drop one.\n"
    "8. If a block titled OPERATOR DIRECTIVE appears after the DATA, it is a human "
    "operator's free-text steering instruction and takes priority over your own "
    "judgement calls — honor it in 'what to change next week'. If it conflicts with what "
    "the DATA supports, say plainly why it couldn't be fully honored rather than "
    "silently ignoring or silently complying; it never licenses a fact not in the DATA.\n\n"

    "OUTPUT FORMAT: Freeform prose (not JSON, no headers), in this exact order: biggest "
    "win, biggest concern, what's working, what to change next week. "
