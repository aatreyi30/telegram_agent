...
import { CHART_SERIES_COLORS } from "@/constants/charts";

const PALETTE = [...CHART_SERIES_COLORS, "hsl(var(--muted-foreground))"];
      ...
      <CardContent className={open ? "space-y-3 pb-4" : "hidden"}>
        ...
        {e.deal_mix && Object.keys(e.deal_mix).length > 0 && (
          <div>
            ...
            <div className="flex flex-wrap gap-1.5">
              {Object.entries(e.deal_mix).map(([t, s]: any) => (
...
export function CompetitorDashboard() {
  ...
  return (
    <div>
      <PageHeader title="Competitor dashboard"
        sub="Direct competitors (platform + Telegram) vs Telegram-only channels — all metrics, side by side." />
        ...
        <Async q={q} rows={2}>
          {(d) => {
            ...
            const wdData = days.map((d) => {
              ...
              entities.forEach((e: any) => { row[e.name] = e.weekday_distribution?.[d] ?? 0; });
              return row;
            });
                        ...
                        <THead>
                          <TR>
                            ...
                            {entities.map((e: any, i: number) => (
                              <TH key={i} className="text-right">{e.name}</TH>
                            ))}
                          </TR>
                        </THead>
                        <TBody>
                            ...
                            return (
                              <TR key={dim.key}>
                                ...
                                {entities.map((e: any, i: number) => {
                                  ...
                                  return (
                                    <TD key={i} className={`text-right font-mono text-xs ${isB ? "font-bold text-green-500" : ""}`}>
