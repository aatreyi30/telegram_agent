...
import { useDay, useGrowth } from "@/queries/queries";
...
/** Per-merchant breakdown for one date (or range) — was its own "/day" page, now a
 * click-in drill-down from the Analytics timeline chart, so "here's the trend" and
 * "here's what happened that day" live in one place instead of a second nav item. */
export function DayDetail({ start, end }: { start: string; end: string }) {
  const q = useDay(start || null, end || null, { enabled: !!start });
  const growth = useGrowth(start || undefined, end || undefined, { enabled: !!start && !!end });
              ...
              <CardHeader>
                ...
                <div className="flex items-center justify-between">
                  <CardTitle className="text-base">
                    Merchants — {isoSlash(d.date)}{"date_end" in d && d.date_end ? ` → ${isoSlash(d.date_end)}` : ""}
                  </CardTitle>
                  {d.baseline && (
