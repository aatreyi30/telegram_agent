...
**Prompt layout — one prompt per file, name = what it produces** (`ai/prompts/`). Each system prompt lives in its own module whose name states its job, so there is no guessing which file holds "deal" vs "loot" instructions: `grounding.py` → `GROUNDING_SYSTEM` (on every call), `daily_plan.py` → `DAILY_PLAN_SYSTEM`, `weekly_plan.py` → `WEEKLY_PLAN_SYSTEM`, `single_deal_post.py` → `SINGLE_DEAL_POST_SYSTEM`, `loot_post.py` → `LOOT_POST_SYSTEM`, `insight_line.py` → `INSIGHT_LINE_SYSTEM`, `competitor_check.py` → `COMPETITOR_CHECK_SYSTEM` (+ `competitor_check_input`), `coach.py` → `COACH_SYSTEM`. All are passed as `system_extra=`; the trace `call` labels below (`day_plan`, `copywriter_deal`, …) are separate runtime IDs in `ai_traces` and unchanged.

| # | Call (fn · file) | Prompt · trace `call` | Trigger | Input (user message) | Output | Params | Persisted |
|---|---|---|---|---|---|---|---|
| 1 | `generate_day_plan` · `ai/planner.py` | `DAILY_PLAN_SYSTEM` · `day_plan` | `daily_plan` job 06:00 · regenerate | yesterday report, 14d trajectory, cadence, `available_deals`, posting windows, deal-type alloc, week theme, directive + **steering history** | digest (**doubles as daily briefing**: went-well/badly + do-today) + `===PLAN===` + `post_slots` JSON | `3200`, effort | `campaign_plans` DAILY (fact-checked) + `ai_traces` |
| 2 | `generate_week_plan` · `ai/planner.py` | `WEEKLY_PLAN_SYSTEM` · `week_plan` | `weekly_report` job Mon · `/plan/weekly` · regenerate | last-week evidence: post-type perf, merchant opps, follower deltas, retro, directive | digest (**doubles as weekly retro**: win/concern/what-to-change) + `daily_themes`(per-day loot/single split)/`direction`/`loot_deal_ratio`/`merchant_priorities` JSON | `2000`, effort | `campaign_plans` WEEKLY (ratio + priorities now persisted, not overwritten by the deterministic pass) + `ai_traces` |
| 3 | `Copywriter.write_for_item` · `ai/copywriter.py` | `SINGLE_DEAL_POST_SYSTEM` · `copywriter_deal` | `jit_fill` (per **deal** slot, 1m) | PRODUCT, FORMAT_REFERENCE (`_deal_examples`), slot theme/merchant, CHANNEL_STYLE; `<link/>` swapped after | `<hook>/<name>/<price>/<cta>` tags → post | `600`, `low` | `generated_posts` + `ai_traces` |
| 4 | `Copywriter.write_for_loot` · `ai/copywriter.py` | `LOOT_POST_SYSTEM` · `copywriter_loot` | `jit_fill` (per **loot** slot) | ITEMS (label → `<LINK_n>`), price cap **and/or floor** (a "₹X–₹Y" band; slots with `min_price`/`max_price` fetch a source-filtered pool), `_loot_examples`, CHANNEL_STYLE | `<theme>/<items>` tags → loot | `600`, `low` | `generated_posts` + `ai_traces` |
| 5 | `insight_writer.narrate` · `ai/insight_writer.py` | `INSIGHT_LINE_SYSTEM` · `narrate` | inside Growth/Reasoning engines (daily) | kind + observation + evidence JSON | one grounded sentence (deterministic fallback on failure) | `120`, `low` | in `growth_*`/`reasoned_insights` + `ai_traces` |
| 6 | `verify_candidate` · `collection/discovery.py` | `COMPETITOR_CHECK_SYSTEM` · `verify_candidate` | `competitor_discover` job 06:30 (only when < `competitor_target` and the match is ambiguous) | brand + candidate handles/titles | JSON `{handle, Direct/Indirect}` | `120` | `competitors` + `ai_traces` |
